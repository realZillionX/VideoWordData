...
JSONL Format:
{
    ...
    "speech_description": "", 
    "audio_description": "", 
    "prompt": "Story beginning + Continuation (Chinese)"
}
...
def clean_dialogue_formatting(text):
    ...
    - Removes newlines that appear right after a closing quote
    - This ensures closing quotes stay with their preceding dialogue
    """
    import re
...
def generate_jsonl_entry(video_path, prompt_text, response_text):
    """Generate a JSONL entry for rendering task (prompt includes continuation)"""
    ...
    # Rendering prompt - includes BOTH intro AND continuation
    prompt = f"中文故事视频。故事开头：{prompt_text} 故事续写：{response_text}"
...
def main(base_dir=None, num_samples=None, start_idx=0, num_workers=None):
    ...
    TASK_JSONL_PATH = DATASET_DIR / f"tinystories_chinese_rendering_video_data_{start_idx}.jsonl"
    ...
    for idx, story_text in enumerate(stories):
        ...
        if len(response_text_clean) > 192:
            ...
            if len(response_text) > limit:
                ...
                if last_punct_idx > 150: # If we don't lose too much content
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate TinyStories Chinese video dataset (Rendering)")
