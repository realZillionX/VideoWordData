...
def generate_jsonl_entry(video_path, prompt_text):
    ...
    prompt = (
        f"A story video on white background. Story beginning in black: "
        f"\"{prompt_text}\". "
...
def main(base_dir=None, num_samples=None, start_idx=0, num_workers=None):
    ...
    TASK_JSONL_PATH = DATASET_DIR / f"tinystories_inference_video_data_{start_idx}.jsonl"
    ...
    for idx, sample in enumerate(dataset):
        ...
        if len(words) > 192:
            # Smart truncation: try to find last punctuation before limit
            limit = 192
            ...
            if last_punct_idx > 150:  # If we don't lose too much
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate TinyStories video dataset (Inference)")
