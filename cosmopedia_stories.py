"""
Cosmopedia Stories for Young Children Dataset to Video Conversion Script
Dataset: https://huggingface.co/datasets/khairi/cosmopedia-stories-young-children
...
DATASET_DIR = BASE_DIR / "cosmopedia_stories"
...
def generate_jsonl_entry(video_path, prompt_text):
    """Generate a JSONL entry for story continuation task"""
    
    visual_desc = (
        f"A static educational video frame showing a children's story reading task. "
    ...
    prompt = (
        f"A children's story video on white background. Story beginning in black: "
        f"\"{prompt_text}\". "
        f"Story continuation appears gradually in black, word by word over {VIDEO_DURATION} seconds."
    )
...
def main(num_samples=None, start_idx=0, num_workers=None):
    ...
    TASK_JSONL_PATH = DATASET_DIR / f"cosmopedia_stories_video_data_{start_idx}.jsonl"
    ...
    print("Loading Cosmopedia Stories for Young Children dataset from HuggingFace...")
    dataset = load_dataset("khairi/cosmopedia-stories-young-children", split="train")
    ...
    for idx, sample in enumerate(dataset):
        # Extract story text - try common field names
        story_text = sample.get("text", "") or sample.get("story", "") or sample.get("content", "")
        ...
        video_filename = f"cosmopedia_stories_{start_idx + idx:06d}.mp4"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate Cosmopedia Stories video dataset")
