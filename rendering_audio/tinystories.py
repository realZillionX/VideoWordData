...
Same as inference but JSONL prompt contains the spoken content.
Used for training text rendering ability.
"""
...
def generate_jsonl_entry(video_path, prompt_text, spoken_text):
    ...
    prompt = f"Story with subtitles: \"{full_content}\""
...
def main(base_dir=None, num_samples=None, start_idx=0, num_workers=None):
    ...
    TASK_JSONL_PATH = DATASET_DIR / f"tinystories_rendering_audio_video_data_{start_idx}.jsonl"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate TinyStories Audio Video dataset (Rendering)")
